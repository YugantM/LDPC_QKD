...
/* INITIALIZE PROBABILITY PROPAGATION.  Stores initial ratios, probabilities,
   and guess at decoding. */
void initprp_log(mod2sparse *H, double *llratio, char *dblk, double *bprb)
{
  mod2entry *e;
  int N;
  int j;
  ...
  for (j = 0; j < N; j++)
  {
    ...
    if (bprb)
      bprb[j] = 1.0 - 1.0 / (1.0 + exp(llratio[j]));
    dblk[j] = llratio[j] >= 0;
  }
}

/* DO ONE ITERATION OF PROBABILITY PROPAGATION. */
void iterprp_log(mod2sparse *H, double *llratio, char *dblk, double *bprb)
{
  double pr, dl, t;
  mod2entry *e;
  int N, M;
