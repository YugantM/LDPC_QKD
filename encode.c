...
)
{
 ...
 "Can't encode if number of bits (%d) not greater than number of checks (%d)\n",
  ...
  /* Create parity bits file. */

  pchkf = open_file_std(parity_bits,"w");
  if (pchkf==NULL)
  ...
  { 
    ...
    { case 's':
      ...
      case 'd':
      { dense_encode (sblk, cblk, pchk,u , v);
        break;
      }
      case 'm':
    ...
    */
    ...
    /* Write parity bits to output file. */

    blockio_write(pchkf,pchk,M);
    if (ferror(pchkf))
    { break;
    }
  ...
  */
  ...
  if (ferror(pchkf) || fclose(pchkf)!=0)
  { fprintf(stderr,"Error writing encoded blocks to %s\n",parity_bits);
    exit(1);
  }
...
void usage(void)
{ fprintf(stderr,
   "Usage:  encode [ -f ] pchk-file gen-file source-file parity-bits \n");
  exit(1);
}
