/* BOB_ENCODE.C - Emulates Encode message blocks. */
...
)
{
  ...
  FILE *srcf, *encf, *pchkf;
  char *sblk, *cblk, *chks, *pchk;
  int i, n;
 ...
 "Can't encode if number of bits (%d) not greater than number of checks (%d)\n",
  ...
  pchkf = open_file_std(parity_bits,"r");
  if (pchkf==NULL)
  ...
  { 
    ...
    if (blockio_read(pchkf,pchk,M)==EOF) 
    { break;
    }
  ...
  fprintf(stderr,
    "Encoded blocks:%d , source block size: %d, encoded block size: %d\n",n,N-M,N);
...
void usage(void)
{ fprintf(stderr,
   "Usage:  encode [ -f ] pchk-file gen-file source-file encoded-file parity-output\n");
  exit(1);
}
